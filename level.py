...
class Level:
    ...
    def __init__(
        ...
        self.player_start_angle = np.pi + np.pi/2
...
level0 = Level(
    tile_size=tile_size,
    ceiling_height=64,
    player_start=np.array([2.5 * tile_size[0], 13.5 * tile_size[1]]),
    player_start_angle=np.pi * (3/2),
    sky_color=(160, 230, 230),
    ground_color=(60, 20, 0)
)

level0.set_walls(walls)
level0.set_floors(floors)
level0.set_ceilings(ceilings)
level0.set_static_objects(static_objects)
